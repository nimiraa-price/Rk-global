...
const curatedSlugs = [
  { slug: 'greece-fip-visa', base: '/digital-nomad' },
  { slug: 'spain-digital-nomad-visa', base: '/digital-nomad' },
  { slug: 'malta-nomad-residence-permit', base: '/digital-nomad' },
  { slug: 'portugal-digital-nomad-visa', base: '/digital-nomad' }
];

const NomadVisasShowcase = () => {
  const [activeCard, setActiveCard] = useState(0);
  const scrollContainerRef = useRef(null);

  // Map slugs to actual program data
  const programs = curatedSlugs.map((curated, index) => {
    const program = nomadPrograms.find(p => p.slug === curated.slug);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const cards = container.querySelectorAll('.program-card-nomad');
      ...
      >
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 flex flex-col items-center">
          ...
          <div className="text-center mb-6 md:mb-10">
            ...
            >
              Work remotely from top international destinations.
            </p>
          </div>
          ...
          >
            ...
            {programs.map((program) => {
              ...
              return (
                <div 
                  key={program.id}
                  id={`nomad-card-${program.id}`}
                  data-id={program.id}
                  onMouseEnter={() => setActiveCard(program.id)}
                  onClick={() => setActiveCard(program.id)}
                  className={`program-card-nomad relative rounded-[2rem] overflow-hidden cursor-pointer shrink-0 snap-center
            ...
            {programs.map((prog) => {
              ...
              <div 
                ...
                onClick={() => {
                  ...
                  document.getElementById(`nomad-card-${prog.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
                }}
...
export default NomadVisasShowcase;
